// buyBunjangProductZeroDelivery.js
// 배송비를 0으로 설정하여 배송비 오류를 피하는 구매 스크립트
...
async function buyBunjangProductZeroDelivery() {
  console.log('🛒 번개장터 상품 구매 스크립트 (배송비 0 설정)');
  ...
  try {
    ...
    console.log(`   - 배송비: ${deliveryPrice.toLocaleString()}원 (0으로 설정)`);
    ...
    // 주문 페이로드 구성 (배송비 0으로 설정)
    const orderPayload = {
      product: {
        id: parseInt(TARGET_PID),
        price: product.price
      },
      deliveryPrice: 0, // 배송비를 0으로 강제 설정
      recipient: {
      ...
      message: "API를 통한 자동 구매 (배송비 0 설정)",
      agreeToTerms: true
    };
    ...
    console.log(`   - 배송비: ${orderPayload.deliveryPrice?.toLocaleString()}원 (0으로 설정)`);
    ...
    console.log('   ⚠️  배송비를 0으로 설정하여 배송비 오류를 피합니다.');
    ...
    console.log(`📋 주문 ID: ${orderResult.id}`);
    console.log('🔧 배송비를 0으로 설정하여 성공했습니다.');
    ...
    if (error.errorCode) {
      ...
      switch (error.errorCode) {
        ...
        case 'INVALID_DELIVERY_PRICE':
          console.error('배송비 오류 - 이미 0으로 설정했는데도 오류 발생');
          break;
        default:
...
async function confirmExecution() {
  ...
  console.log('🔧 배송비를 0으로 설정하여 배송비 오류를 피합니다.');
  console.log('');
  
  console.log('자동으로 구매를 진행합니다...');
  console.log('');
  
  await buyBunjangProductZeroDelivery();
}
...
module.exports = { buyBunjangProductZeroDelivery }; 